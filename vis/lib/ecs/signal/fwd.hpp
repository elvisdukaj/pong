...
template <typename = std::allocator<void>> class basic_dispatcher;

template <typename, typename = std::allocator<void>> class emitter;
...
template <typename Type, typename = std::allocator<void>> class sigh;
 ...
 */
template <auto Candidate> inline constexpr connect_arg_t<Candidate> connect_arg{};
