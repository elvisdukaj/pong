...
template <typename It> class dense_set_iterator final {
	template <typename> friend class dense_set_iterator;
...
template <typename It> class dense_set_local_iterator final {
	template <typename> friend class dense_set_local_iterator;
 ...
 */
template <typename Type, typename Hash, typename KeyEqual, typename Allocator> class dense_set {
	...
	template <typename Other> [[nodiscard]] std::size_t value_to_bucket(const Other& value) const noexcept {
		return fast_mod(static_cast<size_type>(sparse.second()(value)), bucket_count());
	}

	template <typename Other> [[nodiscard]] auto constrained_find(const Other& value, std::size_t bucket) {
	...
	template <typename Other> [[nodiscard]] auto constrained_find(const Other& value, std::size_t bucket) const {
	...
	template <typename Other> [[nodiscard]] auto insert_or_do_nothing(Other&& value) {
...
public:
	...
	size_type erase(const value_type& value) {
		for (size_type* curr = &sparse.first()[value_to_bucket(value)]; *curr != (std::numeric_limits<size_type>::max)();
