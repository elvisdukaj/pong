...
struct poly_inspector {
	 ...
	 */
	template <std::size_t Member, typename... Args> [[nodiscard]] poly_inspector invoke(Args&&... args) const;

	/*! @copydoc invoke */
	template <std::size_t Member, typename... Args> [[nodiscard]] poly_inspector invoke(Args&&... args);
};
 ...
 */
template <typename Concept, std::size_t Len, std::size_t Align> class poly_vtable {
	...
	template <typename... Func> [[nodiscard]] static constexpr auto make_vtable(type_list<Func...>) noexcept {
...
public:
	 ...
	 */
	template <typename Type> [[nodiscard]] static type instance() noexcept {
 ...
 */
template <typename Poly> struct poly_base {
	/**
	...
	/*! @copydoc invoke */
	template <std::size_t Member, typename... Args> [[nodiscard]] decltype(auto) invoke(poly_base& self, Args&&... args) {
 ...
 * @return The return value of the invoked function, if any.
 */
template <std::size_t Member, typename Poly, typename... Args> decltype(auto) poly_call(Poly&& self, Args&&... args) {
