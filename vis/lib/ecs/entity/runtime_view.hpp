...
template <typename Set> class runtime_view_iterator final {
 ...
 */
template <typename Type, typename Allocator> class basic_runtime_view {
...
public:
	...
	[[nodiscard]] bool contains(const entity_type entt) const {
		return !pools.empty() &&
					 ...
					 std::none_of(filter.cbegin(), filter.cend(),
												[entt](const auto* curr) { return curr && curr->contains(entt); });
	}
