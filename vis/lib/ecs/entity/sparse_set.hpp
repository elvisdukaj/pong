...
template <typename Container> struct sparse_set_iterator final {
	...
	constexpr sparse_set_iterator(const Container& ref, const difference_type idx) noexcept : packed{&ref}, offset{idx} {}
...
template <typename Entity, typename Allocator> class basic_sparse_set {
	...
	[[nodiscard]] std::size_t policy_to_head() const noexcept {
		return static_cast<size_type>(
				max_size * static_cast<std::remove_const_t<decltype(max_size)>>(mode != deletion_policy::swap_only));
	}
	...
	void swap_at(const std::size_t lhs, const std::size_t rhs) {
		...
		sparse_ref(from) =
				traits_type::combine(static_cast<typename traits_type::entity_type>(rhs), traits_type::to_integral(from));
		sparse_ref(to) =
				traits_type::combine(static_cast<typename traits_type::entity_type>(lhs), traits_type::to_integral(to));
...
private:
	...
	virtual void swap_or_move([[maybe_unused]] const std::size_t lhs, [[maybe_unused]] const std::size_t rhs) {
		ENTT_ASSERT((mode != deletion_policy::swap_only) || ((lhs < head) == (rhs < head)),
...
protected:
	...
	void in_place_pop(const basic_iterator it) {
		...
		packed[pos] =
				traits_type::combine(static_cast<typename traits_type::entity_type>(std::exchange(head, pos)), tombstone);
	}
	...
	virtual basic_iterator try_emplace(const Entity entt, const bool force_back, const void* = nullptr) {
		...
		case deletion_policy::in_place:
			if (head != max_size && !force_back) {
				...
				elem =
						traits_type::combine(static_cast<typename traits_type::entity_type>(head), traits_type::to_integral(entt));
...
public:
	...
	explicit basic_sparse_set(const type_info& elem, deletion_policy pol = deletion_policy::swap_and_pop,
		...
		ENTT_ASSERT(traits_type::version_mask || mode != deletion_policy::in_place,
								"Policy does not support zero-sized versions");
	}
	 ...
	 */
	template <typename It> iterator push(It first, It last) {
		auto curr = end();
	 ...
	 * @param last An iterator past the last element of the range of entities.
	 */
	template <typename It> void erase(It first, It last) {
	 ...
	 */
	template <typename It> size_type remove(It first, It last) {
		size_type count{};
	...
	void sort_n(const size_type length, Compare compare, Sort algo = Sort{}, Args&&... args) {
		...
		algo(packed.rend() - static_cast<difference_type>(length), packed.rend(), std::move(compare),
	 ...
	 */
	template <typename It> iterator sort_as(It first, It last) {
		ENTT_ASSERT((mode != deletion_policy::in_place) || (head == max_size), "Sorting with tombstones not allowed");
		const size_type len = (mode == deletion_policy::swap_only) ? head : packed.size();
	 ...
	 */
	template <typename Type> void bind(Type&& value) noexcept {
