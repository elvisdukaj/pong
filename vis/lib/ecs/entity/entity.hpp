...
template <typename, typename = void> struct entt_traits;
...
template <> struct entt_traits<std::uint32_t> {
...
template <> struct entt_traits<std::uint64_t> {
 ...
 */
template <typename Traits> class basic_entt_traits {
 ...
 */
template <typename Type> struct entt_traits : basic_entt_traits<internal::entt_traits<Type>> {
	/*! @brief Base type. */
...
struct null_t {
	 ...
	 */
	template <typename Entity> [[nodiscard]] constexpr operator Entity() const noexcept {
	 ...
	 */
	template <typename Entity> [[nodiscard]] constexpr bool operator==(const Entity entity) const noexcept {
		using traits_type = entt_traits<Entity>;
 ...
 * @return False if the two elements differ, true otherwise.
 */
template <typename Entity> [[nodiscard]] constexpr bool operator==(const Entity lhs, const null_t rhs) noexcept {
 ...
 * @return False if the two elements differ, true otherwise.
 */
template <typename Entity> [[nodiscard]] constexpr bool operator==(const Entity lhs, const tombstone_t rhs) noexcept {
	return rhs.operator==(lhs);
}
