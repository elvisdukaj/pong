...
template <typename Container, auto Page> class storage_iterator final {
...
[[nodiscard]] constexpr std::ptrdiff_t operator-(const storage_iterator<Lhs, Page>& lhs,
																								 const storage_iterator<Rhs, Page>& rhs) noexcept {
	return rhs.index() - lhs.index();
}

template <typename Lhs, typename Rhs, auto Page>
[[nodiscard]] constexpr bool operator==(const storage_iterator<Lhs, Page>& lhs,
																				const storage_iterator<Rhs, Page>& rhs) noexcept {
...
template <typename It, typename... Other> class extended_storage_iterator final {
	template <typename Iter, typename... Args> friend class extended_storage_iterator;
...
public:
	...
	template <typename... Args, typename = std::enable_if_t<(!std::is_same_v<Other, Args> && ...) &&
	...
	friend constexpr bool operator==(const extended_storage_iterator<Lhs...>&,
...
class basic_storage
	...
	using container_type = std::vector<typename alloc_traits::pointer,
	...
	template <typename... Args> auto emplace_element(const Entity entt, const bool force_back, Args&&... args) {
		const auto it = base_type::try_emplace(entt, force_back);

		ENTT_TRY {
			auto* elem = to_address(assure_at_least(static_cast<size_type>(it.index())));
			vis::ecs::uninitialized_construct_using_allocator(elem, get_allocator(), std::forward<Args>(args)...);
		}
		ENTT_CATCH {
	...
	void move_to(const std::size_t lhs, const std::size_t rhs) {
		...
		allocator_type allocator{get_allocator()};
		vis::ecs::uninitialized_construct_using_allocator(to_address(assure_at_least(rhs)), allocator, std::move(elem));
...
public:
	...
	using const_iterable =
			iterable_adaptor<internal::extended_storage_iterator<typename base_type::const_iterator, const_iterator>>;
	...
	using const_reverse_iterable = iterable_adaptor<
			internal::extended_storage_iterator<typename base_type::const_reverse_iterator, const_reverse_iterator>>;
	...
	explicit basic_storage(const allocator_type& allocator)
			: base_type{type_id<element_type>(), storage_policy, allocator}, payload{allocator} {}
	 ...
	 */
	template <typename... Args> value_type& emplace(const entity_type entt, Args&&... args) {
	 ...
	 */
	template <typename... Func> value_type& patch(const entity_type entt, Func&&... func) {
	 ...
	 */
	template <typename It> iterator insert(It first, It last, const value_type& value = {}) {
...
public:
	...
	using const_reverse_iterable =
			iterable_adaptor<internal::extended_storage_iterator<typename base_type::const_reverse_iterator>>;
	/*! @brief Storage deletion policy. */
	static constexpr deletion_policy storage_policy{traits_type::in_place_delete};
	 ...
	 * @param allocator The allocator to use.
	 */
	basic_storage(basic_storage&& other, const allocator_type& allocator) : base_type{std::move(other), allocator} {}
	...
	[[nodiscard]] constexpr allocator_type get_allocator() const noexcept {
		...
		if constexpr (std::is_void_v<element_type> &&
									!std::is_constructible_v<allocator_type, typename base_type::allocator_type>) {
	 ...
	 * @param func Valid function objects.
	 */
	template <typename... Func> void patch([[maybe_unused]] const entity_type entt, Func&&... func) {
		ENTT_ASSERT(base_type::contains(entt), "Invalid entity");
...
public:
	 ...
	 * @param allocator The allocator to use.
	 */
	explicit basic_storage(const allocator_type& allocator) : base_type{type_id<void>(), storage_policy, allocator} {}
	...
	entity_type generate(const entity_type hint) {
		if (hint != null && hint != tombstone) {
			if (const auto curr = traits_type::construct(traits_type::to_entity(hint), base_type::current(hint));
					curr == tombstone || !(base_type::index(curr) < base_type::free_list())) {
	 ...
	 * @param last An iterator past the last element of the range to generate.
	 */
	template <typename It> [[deprecated("use ::generate(first, last) instead")]] void insert(It first, It last) {
