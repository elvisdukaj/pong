...
template <typename, typename, typename> class extended_group_iterator;

template <typename It, typename... Owned, typename... Get>
class extended_group_iterator<It, owned_t<Owned...>, get_t<Get...>> {
	template <typename Type> [[nodiscard]] auto index_to_element([[maybe_unused]] Type& cpool) const {
...
public:
	...
	using value_type =
			decltype(std::tuple_cat(std::make_tuple(*std::declval<It>()), std::declval<Owned>().get_as_tuple({})...,
...
class group_handler final : public group_descriptor {
	...
	void common_setup() {
		...
		for (auto first = pools[0u]->rbegin(),
							last = first + static_cast<typename decltype(pools)::difference_type>(pools[0u]->size());
				 first != last; ++first) {
...
public:
	...
	template <std::size_t Index> [[nodiscard]] common_type* storage() const noexcept {
 ...
 */
template <typename... Get, typename... Exclude> class basic_group<owned_t<>, get_t<Get...>, exclude_t<Exclude...>> {
	...
	template <std::size_t... Index> [[nodiscard]] auto pools_for(std::index_sequence<Index...>) const noexcept {
...
public:
	...
	static id_type group_id() noexcept {
		return type_hash<basic_group<owned_t<>, get_t<std::remove_const_t<Get>...>,
...
class basic_group<owned_t<Owned...>, get_t<Get...>, exclude_t<Exclude...>> {
	...
	using base_type =
			std::common_type_t<typename Owned::base_type..., typename Get::base_type..., typename Exclude::base_type...>;
	using underlying_type = typename base_type::entity_type;
