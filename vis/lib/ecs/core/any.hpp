...
enum class any_request : std::uint8_t { transfer, assign, destroy, compare, copy, move, get };
 ...
 */
template <std::size_t Len, std::size_t Align> class basic_any {
	...
	static constexpr bool in_situ =
			(Len != 0u) && alignof(Type) <= Align && sizeof(Type) <= Len && std::is_nothrow_move_constructible_v<Type>;

	template <typename Type>
	static const void* basic_vtable(const request req, const basic_any& value, const void* other) {
		static_assert(!std::is_void_v<Type> && std::is_same_v<std::remove_cv_t<std::remove_reference_t<Type>>, Type>,
									"Invalid type");
		const Type* elem = nullptr;

		if constexpr (in_situ<Type>) {
			elem = (value.mode == any_policy::embedded) ? reinterpret_cast<const Type*>(&value.storage)
		...
		case request::move:
			...
			if constexpr (in_situ<Type>) {
				...
				return ::new (&static_cast<basic_any*>(const_cast<void*>(other))->storage)
						Type{std::move(*const_cast<Type*>(elem))};
			}
			[[fallthrough]];
		case request::get:
			ENTT_ASSERT(value.mode == any_policy::embedded, "Unexpected policy");
	...
	template <typename Type, typename... Args> void initialize([[maybe_unused]] Args&&... args) {
		if constexpr (!std::is_void_v<Type>) {
			...
			if constexpr (std::is_lvalue_reference_v<Type>) {
				...
				if constexpr (std::is_aggregate_v<plain_type> &&
...
public:
	...
	explicit basic_any(std::in_place_type_t<Type>, Args&&... args) : instance{} {
	 ...
	 */
	template <typename Type> explicit basic_any(std::in_place_t, Type* value) : instance{} {
 ...
 */
template <typename Type, std::size_t Len = basic_any<>::length, std::size_t Align = basic_any<Len>::alignment,
					typename... Args>
