...
template <typename Delta> struct basic_process_handler {
...
template <typename Delta, typename Type> struct process_handler final : basic_process_handler<Delta> {
	template <typename... Args> process_handler(Args&&... args) : process{std::forward<Args>(args)...} {}
 ...
 */
template <typename Delta, typename Allocator> class basic_scheduler {
	template <typename Type> using handler_type = internal::process_handler<Delta, Type>;
...
public:
	...
	basic_scheduler() : basic_scheduler{allocator_type{}} {}

	/**
	 * @brief Constructs a scheduler with a given allocator.
	 * @param allocator The allocator to use.
	 */
	explicit basic_scheduler(const allocator_type& allocator) : handlers{allocator, allocator} {}
	 ...
	 */
	basic_scheduler(basic_scheduler&& other) noexcept : handlers{std::move(other.handlers)} {}
	 ...
	 * @return This process scheduler.
	 */
	template <typename Proc, typename... Args> basic_scheduler& attach(Args&&... args) {
		static_assert(std::is_base_of_v<process<Proc, Delta>, Proc>, "Invalid process type");
		auto& ref = handlers.first().emplace_back(
				std::allocate_shared<handler_type<Proc>>(handlers.second(), std::forward<Args>(args)...));
	 ...
	 * @return This process scheduler.
	 */
	template <typename Func> basic_scheduler& attach(Func&& func) {
	 ...
	 * @return This process scheduler.
	 */
	template <typename Proc, typename... Args> basic_scheduler& then(Args&&... args) {
		static_assert(std::is_base_of_v<process<Proc, Delta>, Proc>, "Invalid process type");
	 ...
	 * @return This process scheduler.
	 */
	template <typename Func> basic_scheduler& then(Func&& func) {
		using Proc = process_adaptor<std::decay_t<Func>, Delta>;
