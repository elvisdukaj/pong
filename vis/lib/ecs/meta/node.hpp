...
template <typename Type> [[nodiscard]] auto meta_to_user_traits(const meta_traits traits) noexcept {
	...
	return Type{
			static_cast<std::underlying_type_t<Type>>(static_cast<std::underlying_type_t<meta_traits>>(traits) >> shift)};
}

template <typename Type> [[nodiscard]] auto user_to_meta_traits(const Type value) noexcept {
	static_assert(std::is_enum_v<Type>, "Invalid enum type");
	constexpr auto shift = popcount(static_cast<std::underlying_type_t<meta_traits>>(meta_traits::_user_defined_traits));
	const auto traits =
			static_cast<std::underlying_type_t<internal::meta_traits>>(static_cast<std::underlying_type_t<Type>>(value));
	ENTT_ASSERT(traits <
									((~static_cast<std::underlying_type_t<meta_traits>>(meta_traits::_user_defined_traits)) >> shift),
							"Invalid traits");
...
template <auto Member, typename Type, typename Value> [[nodiscard]] auto* find_member(Type& from, const Value value) {
	for (auto&& elem : from) {
...
[[nodiscard]] inline auto* find_overload(meta_func_node* curr,
																				 std::remove_pointer_t<decltype(meta_func_node::invoke)>* const ref) {
	while ((curr != nullptr) && (curr->invoke != ref)) {
		curr = curr->next.get();
	}
	return curr;
}
...
template <typename Type> meta_type_node resolve(const meta_context&) noexcept;

template <typename... Args>
[[nodiscard]] auto meta_arg_node(const meta_context& context, type_list<Args...>,
																 [[maybe_unused]] const std::size_t index) noexcept {
...
[[nodiscard]] inline const void* try_cast(const meta_context& context, const meta_type_node& from, const type_info& to,
...
[[nodiscard]] inline auto try_convert(const meta_context& context, const meta_type_node& from, const type_info& to,
	...
	if (from.details) {
		...
		for (auto&& curr : from.details->base) {
			if (auto other = try_convert(context, curr.resolve(context), to, arithmetic_or_enum, curr.cast(instance), func);
					other) {
				return other;
			}
		}
...
template <typename Type> [[nodiscard]] meta_type_node resolve(const meta_context& context) noexcept {
	...
	meta_type_node node{&type_id<Type>(),
											...
											(std::is_arithmetic_v<Type> ? meta_traits::is_arithmetic : meta_traits::is_none) |
													...
													(is_complete_v<meta_associative_container_traits<Type>>
															 ? meta_traits::is_associative_container
															 : meta_traits::is_none),
											size_of_v<Type>,
											&resolve<Type>,
	...
	if constexpr (std::is_arithmetic_v<Type>) {
		node.conversion_helper = +[](void* lhs, const void* rhs) {
			return lhs ? static_cast<double>(*static_cast<Type*>(lhs) = static_cast<Type>(*static_cast<const double*>(rhs)))
								 : static_cast<double>(*static_cast<const Type*>(rhs));
		};
	} else if constexpr (std::is_enum_v<Type>) {
		node.conversion_helper = +[](void* lhs, const void* rhs) {
			return lhs ? static_cast<double>(*static_cast<Type*>(lhs) = static_cast<Type>(
																					 static_cast<std::underlying_type_t<Type>>(*static_cast<const double*>(rhs))))
								 : static_cast<double>(*static_cast<const Type*>(rhs));
		};
	}
	...
	if constexpr (!std::is_void_v<Type> && !std::is_function_v<Type>) {
		node.from_void = +[](const meta_ctx& ctx, void* elem, const void* celem) {
			...
			return meta_any{ctx, std::in_place_type<const std::decay_t<Type>&>,
											*static_cast<const std::decay_t<Type>*>(celem)};
		};
	}

	if constexpr (is_complete_v<meta_template_traits<Type>>) {
		node.templ = meta_template_node{
				meta_template_traits<Type>::args_type::size, &resolve<typename meta_template_traits<Type>::class_type>,
