...
public:
	...
	meta_sequence_container(const meta_ctx& area, Type& instance) noexcept
			: ctx{&area},
				...
				const_reference_node{
						&internal::resolve<std::remove_const_t<std::remove_reference_t<typename Type::const_reference>>>},
...
class meta_any {
	...
	template <typename Type>
	static void basic_vtable([[maybe_unused]] const internal::meta_traits req, [[maybe_unused]] const meta_ctx& area,
													 [[maybe_unused]] const void* value, [[maybe_unused]] void* other) {
		...
		if constexpr (is_complete_v<meta_sequence_container_traits<Type>>) {
			if (!!(req & internal::meta_traits::is_sequence_container)) {
				...
				*static_cast<meta_sequence_container*>(other) =
						!!(req & internal::meta_traits::is_const)
								? meta_sequence_container{area, *static_cast<const Type*>(value)}
								: meta_sequence_container{area, *static_cast<Type*>(const_cast<void*>(value))};
			}
		}
		...
		if constexpr (is_complete_v<meta_associative_container_traits<Type>>) {
			if (!!(req & internal::meta_traits::is_associative_container)) {
				// NOLINTNEXTLINE(bugprone-casting-through-void)
				*static_cast<meta_associative_container*>(other) =
						!!(req & internal::meta_traits::is_const)
								? meta_associative_container{area, *static_cast<const Type*>(value)}
								: meta_associative_container{area, *static_cast<Type*>(const_cast<void*>(value))};
			}
		}
	}
	...
	meta_any(const meta_any& other, any ref) noexcept : storage{std::move(ref)}, ctx{other.ctx} {
...
public:
	 ...
	 * @param area The context from which to search for meta types.
	 */
	meta_any(meta_ctx_arg_t, const meta_ctx& area) : ctx{&area} {}
	 ...
	 */
	template <typename Type>
	explicit meta_any(std::in_place_t, Type* value) : meta_any{locator<meta_ctx>::value_or(), std::in_place, value} {}
	 ...
	 */
	template <typename Type>
	explicit meta_any(const meta_ctx& area, std::in_place_t, Type* value) : storage{std::in_place, value}, ctx{&area} {
		if (storage) {
	...
	meta_any(const meta_ctx& area, meta_any&& other)
			: storage{std::move(other.storage)},
				ctx{&area},
				node{(other.node.resolve != nullptr)
								 ? std::exchange(other.node, internal::meta_type_node{}).resolve(internal::meta_context::from(*ctx))
								 : std::exchange(other.node, internal::meta_type_node{})},
	 ...
	 */
	template <typename... Args> meta_any invoke(id_type id, Args&&... args) const;

	/*! @copydoc invoke */
	template <typename... Args> meta_any invoke(id_type id, Args&&... args);
	 ...
	 */
	template <typename Type> bool set(id_type id, Type&& value);
	 ...
	 */
	template <typename Type> [[nodiscard]] const Type* try_cast() const {
	...
	/*! @copydoc try_cast */
	template <typename Type> [[nodiscard]] Type* try_cast() {
		if constexpr (std::is_const_v<Type>) {
			...
			return static_cast<Type*>(
					const_cast<void*>(internal::try_cast(internal::meta_context::from(*ctx), node, other, storage.data())));
		}
	}
	 ...
	 */
	template <typename Type> [[nodiscard]] std::remove_const_t<Type> cast() const {
	...
	/*! @copydoc cast */
	template <typename Type> [[nodiscard]] std::remove_const_t<Type> cast() {
	 ...
	 * invalid one otherwise.
	 */
	template <typename Type> [[nodiscard]] meta_any allow_cast() const {
		if constexpr (std::is_reference_v<Type> && !std::is_const_v<std::remove_reference_t<Type>>) {
			return meta_any{meta_ctx_arg, *ctx};
		} else {
			// also support early return for performance reasons
			return ((node.info != nullptr) && (*node.info == vis::ecs::type_id<Type>()))
								 ? as_ref()
								 : allow_cast(meta_type{*ctx, internal::resolve<std::remove_cv_t<std::remove_reference_t<Type>>>(
																									internal::meta_context::from(*ctx))});
		}
	}
	 ...
	 * @return True if there exists a viable conversion, false otherwise.
	 */
	template <typename Type> [[nodiscard]] bool allow_cast() {
		if constexpr (std::is_reference_v<Type> && !std::is_const_v<std::remove_reference_t<Type>>) {
	...
	/*! @copydoc any::emplace */
	template <typename Type, typename... Args> void emplace(Args&&... args) {
		release();
	...
	[[nodiscard]] meta_sequence_container as_sequence_container() noexcept {
		meta_sequence_container proxy = (storage.policy() == any_policy::cref)
																				? std::as_const(*this).as_sequence_container()
																				: meta_sequence_container{};
		if (!proxy && vtable != nullptr) {
			vtable(internal::meta_traits::is_sequence_container, *ctx, storage.data(), &proxy);
		}
		return proxy;
	}
	...
	[[nodiscard]] meta_sequence_container as_sequence_container() const noexcept {
		...
		if (vtable != nullptr) {
			vtable(internal::meta_traits::is_sequence_container | internal::meta_traits::is_const, *ctx, storage.data(),
						 &proxy);
		}
		return proxy;
	}
	...
	[[nodiscard]] meta_associative_container as_associative_container() noexcept {
		meta_associative_container proxy = (storage.policy() == any_policy::cref)
																					 ? std::as_const(*this).as_associative_container()
																					 : meta_associative_container{};
		if (!proxy && vtable != nullptr) {
			vtable(internal::meta_traits::is_associative_container, *ctx, storage.data(), &proxy);
		}
		return proxy;
	}
	...
	[[nodiscard]] meta_associative_container as_associative_container() const noexcept {
		...
		if (vtable != nullptr) {
			vtable(internal::meta_traits::is_associative_container | internal::meta_traits::is_const, *ctx, storage.data(),
						 &proxy);
		}
		return proxy;
	}
	...
	[[nodiscard]] bool operator==(const meta_any& other) const noexcept {
		return (ctx == other.ctx) && (((node.info == nullptr) && (other.node.info == nullptr)) ||
																	((node.info != nullptr) && (other.node.info != nullptr) &&
																	 *node.info == *other.node.info && storage == other.storage));
	}
 ...
 */
template <typename Type> [[nodiscard]] meta_any forward_as_meta(const meta_ctx& ctx, Type&& value) {
 ...
 * @return A properly initialized and not necessarily owning wrapper.
 */
template <typename Type> [[nodiscard]] meta_any forward_as_meta(Type&& value) {
...
struct meta_handle {
	 ...
	 * @param area The context from which to search for meta types.
	 */
	meta_handle(meta_ctx_arg_t, const meta_ctx& area) : any{meta_ctx_arg, area} {}

	/**
	 * @brief Creates a handle that points to an unmanaged object.
	 * @param value An instance of an object to use to initialize the handle.
	 */
	meta_handle(meta_any& value) : any{value.as_ref()} {}

	/**
	 * @brief Creates a handle that points to an unmanaged object.
	 * @param value An instance of an object to use to initialize the handle.
	 */
	meta_handle(const meta_any& value) : any{value.as_ref()} {}
	...
	meta_handle(const meta_ctx& ctx, Type& value) : any{ctx, std::in_place_type<Type&>, value} {}
	 ...
	 */
	template <typename Type, typename = std::enable_if_t<!std::is_same_v<std::decay_t<Type>, meta_handle>>>
	meta_handle(Type& value) : any{std::in_place_type<Type&>, value} {}
...
struct meta_custom {
	 ...
	 */
	meta_custom(internal::meta_custom_node curr) noexcept : node{std::move(curr)} {}

	/**
	 * @brief Generic conversion operator.
	 * @tparam Type Type to which conversion is requested.
	 */
	template <typename Type> [[nodiscard]] operator Type*() const noexcept {
...
struct meta_data {
	 ...
	 * @param curr The underlying node with which to construct the instance.
	 */
	meta_data(const meta_ctx& area, internal::meta_data_node curr) noexcept : node{std::move(curr)}, ctx{&area} {}
	...
	bool set(meta_handle instance, Type&& value) const {
		return (node.set != nullptr) &&
					 node.set(meta_handle{*ctx, std::move(instance)}, meta_any{*ctx, std::forward<Type>(value)});
	}
	 ...
	 */
	template <typename Type> [[nodiscard]] Type traits() const noexcept {
...
struct meta_func {
	 ...
	 * @param curr The underlying node with which to construct the instance.
	 */
	meta_func(const meta_ctx& area, internal::meta_func_node curr) noexcept : node{std::move(curr)}, ctx{&area} {}
	...
	meta_any invoke(meta_handle instance, meta_any* const args, const size_type sz) const {
		return ((node.invoke != nullptr) && (sz == arity())) ? node.invoke(meta_handle{*ctx, std::move(instance)}, args)
	...
	meta_any invoke(meta_handle instance, Args&&... args) const {
		return invoke(std::move(instance),
									std::array<meta_any, sizeof...(Args)>{meta_any{*ctx, std::forward<Args>(args)}...}.data(),
									sizeof...(Args));
	}
				...
				for (; pos < sz && args[pos]; ++pos) {
					...
					if (const auto& info = other.info(); info == type.info()) {
						++match;
										 ...
										 !(type.node.details &&
											 (internal::find_member<&internal::meta_base_node::type>(type.node.details->base, info.hash()) ||
												internal::find_member<&internal::meta_conv_node::type>(type.node.details->conv,
																																							 info.hash())))) {
						break;
					}
				}
...
public:
	 ...
	 * @param curr The underlying node with which to construct the instance.
	 */
	meta_type(const meta_ctx& area, internal::meta_type_node curr) noexcept : node{std::move(curr)}, ctx{&area} {}
	...
	[[nodiscard]] meta_type remove_pointer() const noexcept {
		return (node.remove_pointer != nullptr) ? meta_type{*ctx, node.remove_pointer(internal::meta_context::from(*ctx))}
																						: *this;
	}
	...
	[[nodiscard]] meta_type template_type() const noexcept {
		return (node.templ.resolve != nullptr) ? meta_type{*ctx, node.templ.resolve(internal::meta_context::from(*ctx))}
																					 : meta_type{};
	}
	...
	[[nodiscard]] meta_type template_arg(const size_type index) const noexcept {
		return index < template_arity() ? meta_type{*ctx, node.templ.arg(internal::meta_context::from(*ctx), index)}
																		: meta_type{};
	}
	...
	[[nodiscard]] bool can_convert(const meta_type& other) const noexcept {
		return (internal::try_convert(
								internal::meta_context::from(*ctx), node, other.info(), other.is_arithmetic() || other.is_enum(),
								nullptr, [](const void*, auto&&... args) { return ((static_cast<void>(args), 1) + ... + 0u); }) != 0u);
	}
	 ...
	 */
	[[nodiscard]] meta_range<meta_type, typename decltype(internal::meta_type_descriptor::base)::const_iterator>
	base() const noexcept {
		using range_type = meta_range<meta_type, typename decltype(internal::meta_type_descriptor::base)::const_iterator>;
		return node.details ? range_type{{*ctx, node.details->base.cbegin()}, {*ctx, node.details->base.cend()}}
												: range_type{};
	}
	 ...
	 */
	[[nodiscard]] meta_range<meta_data, typename decltype(internal::meta_type_descriptor::data)::const_iterator>
	data() const noexcept {
		using range_type = meta_range<meta_data, typename decltype(internal::meta_type_descriptor::data)::const_iterator>;
		return node.details ? range_type{{*ctx, node.details->data.cbegin()}, {*ctx, node.details->data.cend()}}
												: range_type{};
	}
	...
	[[nodiscard]] meta_data data(const id_type id) const {
		const auto* elem =
				internal::look_for<&internal::meta_type_descriptor::data>(internal::meta_context::from(*ctx), node, id);
	 ...
	 */
	[[nodiscard]] meta_range<meta_func, typename decltype(internal::meta_type_descriptor::func)::const_iterator>
	func() const noexcept {
		using return_type = meta_range<meta_func, typename decltype(internal::meta_type_descriptor::func)::const_iterator>;
		return node.details ? return_type{{*ctx, node.details->func.cbegin()}, {*ctx, node.details->func.cend()}}
												: return_type{};
	}
	...
	[[nodiscard]] meta_func func(const id_type id) const {
		const auto* elem =
				internal::look_for<&internal::meta_type_descriptor::func>(internal::meta_context::from(*ctx), node, id);
	 ...
	 * @return A wrapper containing the new instance, if any.
	 */
	template <typename... Args> [[nodiscard]] meta_any construct(Args&&... args) const {
		return construct(std::array<meta_any, sizeof...(Args)>{meta_any{*ctx, std::forward<Args>(args)}...}.data(),
										 sizeof...(Args));
	...
	[[nodiscard]] meta_any from_void(void* elem, bool transfer_ownership = false) const {
		return ((elem != nullptr) && (node.from_void != nullptr))
							 ? node.from_void(*ctx, elem, transfer_ownership ? elem : nullptr)
							 : meta_any{meta_ctx_arg, *ctx};
	}
	...
	[[nodiscard]] meta_any from_void(const void* elem) const {
		return ((elem != nullptr) && (node.from_void != nullptr)) ? node.from_void(*ctx, nullptr, elem)
																															: meta_any{meta_ctx_arg, *ctx};
	}
	...
	meta_any invoke(const id_type id, meta_handle instance, meta_any* const args, const size_type sz) const {
		if (node.details) {
			if (auto* elem = internal::find_member<&internal::meta_func_node::id>(node.details->func, id); elem != nullptr) {
				if (const auto* candidate = lookup(args, sz, (instance->base().policy() == any_policy::cref),
																					 [curr = elem]() mutable {
																						 return (curr != nullptr) ? std::exchange(curr, curr->next.get()) : nullptr;
																					 });
						candidate) {
	...
	meta_any invoke(const id_type id, meta_handle instance, Args&&... args) const {
		return invoke(id, std::move(instance),
									std::array<meta_any, sizeof...(Args)>{meta_any{*ctx, std::forward<Args>(args)}...}.data(),
									sizeof...(Args));
	}
	...
	[[nodiscard]] bool operator==(const meta_type& other) const noexcept {
		...
		return (ctx == other.ctx) && ((node.info == nullptr) == (other.node.info == nullptr)) &&
					 (node.info == nullptr || (*node.info == *other.node.info));
	}
...
template <typename... Args> meta_any meta_any::invoke(const id_type id, Args&&... args) {
	return type().invoke(id, *this, std::forward<Args>(args)...);
}

template <typename Type> bool meta_any::set(const id_type id, Type&& value) {
...
[[nodiscard]] inline meta_any meta_any::allow_cast(const meta_type& type) const {
	return internal::try_convert(
			internal::meta_context::from(*ctx), node, type.info(), type.is_arithmetic() || type.is_enum(), storage.data(),
			[this, &type]([[maybe_unused]] const void* instance, [[maybe_unused]] auto&&... args) {
				if constexpr ((std::is_same_v<std::remove_const_t<std::remove_reference_t<decltype(args)>>,
...
class meta_sequence_container::meta_iterator final {
	...
	template <typename It> static void basic_vtable(const void* value, const std::ptrdiff_t offset, meta_any* other) {
...
[[nodiscard]] inline meta_type meta_sequence_container::value_type() const noexcept {
	return (value_type_node != nullptr) ? meta_type{*ctx, value_type_node(internal::meta_context::from(*ctx))}
																			: meta_type{};
}
...
inline meta_sequence_container::iterator meta_sequence_container::insert(const iterator& it, meta_any value) {
	...
	if (const auto vtype = value_type_node(internal::meta_context::from(*ctx));
			!const_only && (value.allow_cast({*ctx, vtype}) ||
											value.allow_cast({*ctx, const_reference_node(internal::meta_context::from(*ctx))}))) {
		const bool is_value_type = (value.type().info() == *vtype.info);
		return insert_fn(*ctx, const_cast<void*>(data), is_value_type ? value.base().data() : nullptr,
										 is_value_type ? nullptr : value.base().data(), it);
	}
...
[[nodiscard]] inline meta_type meta_associative_container::mapped_type() const noexcept {
	return (mapped_type_node != nullptr) ? meta_type{*ctx, mapped_type_node(internal::meta_context::from(*ctx))}
																			 : meta_type{};
}
...
inline bool meta_associative_container::insert(meta_any key, meta_any value = {}) {
	return !const_only && key.allow_cast(meta_type{*ctx, key_type_node(internal::meta_context::from(*ctx))}) &&
				 ((mapped_type_node == nullptr) ||
					value.allow_cast(meta_type{*ctx, mapped_type_node(internal::meta_context::from(*ctx))})) &&
...
inline meta_associative_container::size_type meta_associative_container::erase(meta_any key) {
	return (!const_only && key.allow_cast(meta_type{*ctx, key_type_node(internal::meta_context::from(*ctx))}))
						 ? erase_fn(const_cast<void*>(data), key.base().data())
						 : 0u;
}
...
[[nodiscard]] inline meta_associative_container::iterator meta_associative_container::find(meta_any key) {
	return key.allow_cast(meta_type{*ctx, key_type_node(internal::meta_context::from(*ctx))})
						 ? find_fn(*ctx, const_only ? nullptr : const_cast<void*>(data), data, key.base().data())
						 : iterator{};
}
