 ...
 */
template <typename Type> [[nodiscard]] meta_type resolve(const meta_ctx& ctx) noexcept {
 ...
 * @return The meta type associated with the given type, if any.
 */
template <typename Type> [[nodiscard]] meta_type resolve() noexcept {
 ...
 */
[[nodiscard]] inline meta_range<meta_type, typename decltype(internal::meta_context::value)::const_iterator>
resolve(const meta_ctx& ctx) noexcept {
	auto&& context = internal::meta_context::from(ctx);
